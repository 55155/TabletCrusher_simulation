...
Samplebag_filename = "/home/seongjin/Desktop/Seongjin/genesis_simulation_on_linux/My_asset/Samplebag_v2.obj"
...
cloth_material = gs.materials.PBD.Cloth(rho=10.0,)
######################### entities ##########################
...
Sample_holder = scene.add_entity(
    material=rigid_material,
    morph=gs.morphs.Mesh(
        pos=(0.0000, 0.0000, 0.0000),
        file="/home/seongjin/Desktop/Seongjin/genesis_simulation_on_linux/My_asset/Samplebag_holder.obj",
        scale=0.02,
...
qpos = franka.inverse_kinematics(
    link=end_effector,
    pos=np.array([0.000, 0.000, .5]) if args.n_envs == 0 else np.array([[0.0000, 0.0000, .5]] * args.n_envs),
    quat=np.array([0, 1, 0, 0]) if args.n_envs == 0 else np.array([[0, 1, 0, 0]] * args.n_envs,),
)
qpos[..., -2:] = 0.04
qpos[6] = 2.8  # wrist joint adjustment

# Samplebag.fix_particles(Samplebag.find_closest_particle((0.0, -1, 0.7)))
# Samplebag.fix_particles(Samplebag.find_closest_particle((0.0, 1, 0.7)))
# Samplebag.fix_particles(Samplebag.find_closest_particle((0.0, -1, 0)))
# Samplebag.fix_particles(Samplebag.find_closest_particle((0.0, 1, 0)))
...
qpos = franka.inverse_kinematics(
    link=end_effector,
    pos=np.array([0.0, 0.00, 0.2]) if args.n_envs == 0 else np.array([[0.000, 0.000, 0.2]] * args.n_envs),
    quat=np.array([0, 1, 0, 0]) if args.n_envs == 0 else np.array([[0, 1, 0, 0]] * args.n_envs,),
)
qpos[..., -2:] = 0.1
qpos[6] = 2.8  # wrist joint adjustment
...
for i in range(horizon):
    ...
    print("Scene stepped.")
    cam.render()
cam.stop_recording(save_to_filename="./video/ipc_solver_samplebag/[20260123] LagacyCoupler_Samplebag-Robotarm interaction 1.mp4")
